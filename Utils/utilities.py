...
import json
import pandas as pd
from running_models import EnhancedAdaboost
import pickle 
TEST = True
if not TEST:
    DATA_FOLDER = "data/"
...
def print_summary_table(summary_data, model_classes):
    ...
    model_col_width = 28 # Wide enough for "Enhanced_Adaboost"
...
def run_deployment_simulation(training_project: str, test_file_path: str, use_pretrained=True):
    ...
    if use_pretrained:
        model = load_model(EnhancedAdaboost, training_project)
